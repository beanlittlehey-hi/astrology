...
- 冻结规则还包括 CSS 定位方式：不得通过后续覆盖选择器把 `.custom-nav` 从 `position: fixed` 改为 `relative/absolute/static`，不得让导航栏重新进入页面文档流，避免内容区 `padding-top: navLayout.navTotalHeight` 被重复计算。
- 改 UI 前优先读取：
